...
{
    public partial class AjaxAdminPanelProcess : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        gvDisplayScholarship.Columns[12].Visible = true;
                    }
                    else
                    {
                        gvDisplayScholarship.Columns[12].Visible = false;
                    }
        ...
        public DataSet GetAdminPanelLoadProcess(string MainCategory, string Key, string AcyearId)
        {
            ...
            try
            {
                ...
                strQuery = strQuery + " R.IFSC_Code,P.Scholarship_Issued_AccNo,P.User_ID,P.Status,P.Update_Date,P.Scholarship_Id,P.Suggesred_Date,P.Scholar_Reject,P.DDCheque_No,";



                strQuery = strQuery + " case P.Status when 'Registered' then P.Scholarship_Id else (Select Max(Scholarship_Id) + 1 as Id from t_Registration_Process where Application_Id = R.Application_Id) end  as SuggestScholarshipId  ,";
                strQuery = strQuery +  " case P.Status when 'Registered' then concat('Suggest-',P.Scholarship_Id) else concat('Suggest-',(Select Max(Scholarship_Id) + 1 as Id from t_Registration_Process where Application_Id = R.Application_Id)) end  as SuggestText  ,";


               strQuery = strQuery + " case P.Status when 'Registered' then 'True' when 'Waiting' then 'False' when 'Rejected' then 'False' else 'True' end as SuggestLinkEnable,";
                strQuery = strQuery + " case P.Status when 'Waiting' then 'False' when 'Registered' then 'False' when 'Rejected' then 'False' else 'False' end as lblSuggested ,";
                strQuery = strQuery + " case P.Scholar_Reject when 'Waiting' then 'False' when 'Registered' then 'False' when 'Rejected' then 'True' else  'False' end as lblReject, ";
                strQuery = strQuery + " case P.Status when 'Completed' then 'True' when 'Waiting' then 'True' when 'Registered' then 'False' when 'Rejected' then 'False' else  'False' end as lblCompleted ";
                ...
                if (MainCategory == "Scholarship No")
                {
                    ...
                    strQuery = strQuery + " Where P.Scholarship_No like '%" + Key + "%'";
                }
                else if (MainCategory == "Cheque No")
                {

                    strQuery = strQuery + " Where P.DDCheque_No like '%" + Key + "%'";
                }
                else
                {
                    //strQuery = strQuery + " Where (P.Status in ('Registered','Waiting') or P.Scholar_Reject in('Rejected')) and P.IsUpload_Status='0' and P.IsVerify='0' ";
                    strQuery = strQuery + " Where  P.IsUpload_Status='0' and P.IsVerify='0' ";
                ...
                strQuery = strQuery + " order by SuggestLinkEnable desc";
